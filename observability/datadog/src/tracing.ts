...
export class DatadogExporter extends BaseExporter {
  ...
  private tryEmitReadySpans(traceId: string): void {
    ...
    if (!state.treeEmitted) {
      ...
      if (tree) {
        const inferredEndTimes = this.inferEndTimes(tree);
        this.emitSpanTree(tree, state, inferredEndTimes);
      }
  ...
  /**
   * Infers end times for spans that lack endTime by using the span tree structure.
   * When spans are exported in a batch, using new Date() gives all spans the same end time.
   * This derives per-span end times from: explicit endTime, children's max end time,
   * or next sibling's start time (sequential execution).
   */
  private inferEndTimes(node: SpanNode): Map<string, Date> {
    const inferred = new Map<string, Date>();

    const visit = (n: SpanNode, nextSiblingStartTime?: Date): Date => {
      const childEndTimes: Date[] = [];
      for (let i = 0; i < n.children.length; i++) {
        const child = n.children[i];
        const nextSibling = n.children[i + 1];
        const nextStart = nextSibling ? toDate(nextSibling.span.startTime) : undefined;
        const childEnd = visit(child, nextStart);
        childEndTimes.push(childEnd);
      }

      const childrenMaxEnd =
        childEndTimes.length > 0
          ? new Date(Math.max(...childEndTimes.map((d) => d.getTime())))
          : undefined;

      let endTime: Date;
      if (n.span.endTime) {
        endTime = toDate(n.span.endTime);
      } else if (n.span.isEvent) {
        endTime = toDate(n.span.startTime);
      } else if (childrenMaxEnd) {
        endTime = childrenMaxEnd;
      } else if (nextSiblingStartTime) {
        endTime = nextSiblingStartTime;
      } else {
        endTime = toDate(n.span.startTime);
      }

      inferred.set(n.span.id, endTime);
      return endTime;
    };
   ...
   * When inferredEndTime is provided (from tree emission), use it instead of new Date()
   * to avoid all spans sharing the same export-time end.
   */
  private buildSpanOptions(span: AnyExportedSpan, inferredEndTime?: Date): LLMObsSpanOptions {
   ...
   */
  private emitSpanTree(node: SpanNode, state: TraceState, inferredEndTimes: Map<string, Date>): void {
    const span = node.span;
    const options = this.buildSpanOptions(span, inferredEndTimes.get(span.id));
    ...
    tracer.llmobs.trace(options as any, (ddSpan: any) => {
      ...
      for (const child of node.children) {
        this.emitSpanTree(child, state, inferredEndTimes);
      }
    });
  }
